...
export class PostService {
  ...
  addPost(title: string, content: string, file: File): void {
    ...
    this.httpClient
      ...
      .subscribe((response: PostFromServer) => {
        ...
        const newPost: Post = { title, content, id: response._id, filePath: response.filePath, creator: null };
  ...
  updatePost(id: string, title: string, content: string, fileOrPath: File | string) {
    ...
    if (typeof (fileOrPath === 'object')) {
      ...
      updatedPost = { id, title, content, filePath: fileOrPath as string, creator: null };
    }
    this.httpClient
